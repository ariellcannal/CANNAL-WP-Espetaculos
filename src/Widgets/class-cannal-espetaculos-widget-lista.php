...
class Cannal_Espetaculos_Widget_Lista extends WP_Widget
{
    ...
    private string $nome = 'CANNAL - Lista de Espetáculos';

    private string $descricao = 'Exibe uma lista de espetáculos filtrados por status de temporada.';
    ...
    {
        parent::__construct('cannal_espetaculos_lista', $this->nome, array(
    ...
    public function widget($args, $instance)
    {
        ...
        if (! empty($espetaculos)) {
            ...
            foreach ($espetaculos as $espetaculo_data) {
                ...
                $thumbnail = get_the_post_thumbnail($espetaculo_id, 'thumbnail', array(
                    'class' => 'alignleft'
                ));
    ...
    public function form($instance)
    {
        ...
        ?>
<small><?php echo $this->descricao?></small>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>">Título:</label> <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
</p>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'filtro' ) ); ?>">Filtro:</label> <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'filtro' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'filtro' ) ); ?>">
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'limite' ) ); ?>">Limite de espetáculos:</label> <input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'limite' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'limite' ) ); ?>" type="number" step="1" min="1" value="<?php echo esc_attr( $limite ); ?>" size="3">
</p>
<?php
    }
