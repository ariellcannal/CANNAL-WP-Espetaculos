...
class CANNALEspetaculos_WidgetSidebar extends WP_Widget
{

    public function __construct()
    {
        parent::__construct('cannal_dados_espetaculo', __('CANNAL - Sidebar do Espetáculo', 'cannal-espetaculos'), array(
            'description' => __('Exibe os dados do espetáculo. Funciona apenas em páginas single-espetaculo.', 'cannal-espetaculos'),
            'classname' => 'cannal-widget-espetaculo-sidebar'
        ));
    }
...
<?php
    ...
    public function widget($args, $instance)
    {
        ...
        if ($temporada) {
            ...
            $template = 'widget-sidebar-espetaculo';
            ...
            // não tem temporada ativa - tentar próximas temporadas, senão últimas temporadas
            $temporadas = CANNALEspetaculos_Public::get_proximas_temporadas_static($espetaculo_id, 5);
            if (!empty($temporadas)) {
            } else {
            ...
            if (!empty($temporadas)) {
                $template = 'espetaculo-temporadas';
                $template_vars = compact('temporadas', 'titulo', 'espetaculo_id', 'temporada', 'autor', 'diretor', 'elenco', 'duracao', 'ano_estreia', 'classificacao', 'classificacao_text');
                extract($template_vars);
            }
        }
        ...
        if ($template) {
            ...
            include CANNAL_ESPETACULOS_PLUGIN_DIR . 'templates/public/' . $template . 'php';
