...
class Cannal_Widget_Dados_Espetaculo extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'cannal_dados_espetaculo',
            __( 'CANNAL - Dados do Espetáculo', 'cannal-espetaculos' ),
            array(
    ...
    public function form( $instance ) {
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'titulo_opcao' ) ); ?>"
                    name="<?php echo esc_attr( $this->get_field_name( 'titulo_opcao' ) ); ?>"
                    class="widefat cannal-titulo-opcao-select">
                <option value="informacoes" <?php selected( $titulo_opcao, 'informacoes' ); ?>>
                    <?php esc_html_e( 'Informações (padrão)', 'cannal-espetaculos' ); ?>
                </option>
        ...
        <p class="cannal-titulo-custom-wrap" <?php echo $titulo_opcao !== 'custom' ? 'style="display:none;"' : ''; ?>>
            ...
            <input type="text"
                   id="<?php echo esc_attr( $this->get_field_id( 'titulo_custom' ) ); ?>"
                   name="<?php echo esc_attr( $this->get_field_name( 'titulo_custom' ) ); ?>"
                   value="<?php echo esc_attr( $titulo_custom ); ?>"
                   class="widefat" />
        </p>
        <script>
        (function($){
    ...
    public function widget( $args, $instance ) {
        ...
        // --- Temporada ativa ---
        $temporada = Cannal_Espetaculos_Public::get_active_temporada_static( $espetaculo_id );
        ...
        switch ( $titulo_opcao ) {
            case 'informacoes':
                $titulo = __( 'Informações', 'cannal-espetaculos' );
                break;
            case 'nome_espetaculo':
                $titulo = get_the_title( $espetaculo_id );
                break;
            case 'custom':
                $titulo = $titulo_custom ?: __( 'Informações', 'cannal-espetaculos' );
                break;
            case 'sem_titulo':
            default:
                $titulo = '';
        ...
        $template_vars = compact(
            'titulo', 'espetaculo_id', 'temporada',
            'autor', 'diretor', 'elenco', 'duracao', 'ano_estreia',
            'classificacao', 'classificacao_text',
            'teatro_nome', 'teatro_endereco', 'dias_horarios', 'tipo_sessao',
            'valores', 'link_vendas', 'link_texto'
        );
        ...
        if ( $temporada ) {
            ...
            $proximas = Cannal_Espetaculos_Public::get_proximas_temporadas_static( $espetaculo_id, 5 );

            if ( ! empty( $proximas ) ) {
                // Renderizar widget de próximas apresentações inline
                $widget_proximas = new Cannal_Widget_Proximas_Apresentacoes();
                $widget_proximas->widget(
                    array( 'before_widget' => '', 'after_widget' => '', 'before_title' => '', 'after_title' => '' ),
                    array( 'titulo' => '' )
                );
            } else {
                // Renderizar widget de últimas apresentações inline
                $ultimas = Cannal_Espetaculos_Public::get_ultimas_temporadas_static( $espetaculo_id, 5 );
                if ( ! empty( $ultimas ) ) {
                    $widget_ultimas = new Cannal_Widget_Ultimas_Apresentacoes();
                    $widget_ultimas->widget(
                        array( 'before_widget' => '', 'after_widget' => '', 'before_title' => '', 'after_title' => '' ),
                        array( 'titulo' => '' )
                    );
                }
