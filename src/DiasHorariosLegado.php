...
class CANNALEspetaculos_DiasHorarios
{
    ...
    private static function formatar_grupo_avulso($grupo)
    {
        ...
        if (count($dias) >= 3 && self::sao_consecutivos($dias)) {
            ...
            return "De <span class='dias'>{$primeiro} a {$ultimo} de {$mes_nome}</span> às <span class='horarios'>{$horario}h</span>";
        } else {
            // Listar dias individualmente
            $dias_texto = self::formatar_lista_dias($dias);

            if (count($dias) === 1) {
                return "<span class='dias'>{$dias_texto} de {$mes_nome}</span> às <span class='horarios'>{$horario}h</span>";
            } else {
                return "Dias <span class='dias'>{$dias_texto} de {$mes_nome}</span> às <span class='horarios'>{$horario}h</span>";
            }
        }
    }
    ...
    private static function formatar_lista_dias($dias)
    {
        ...
        return "<span class='dias'>".$texto."</span>";
    }
    ...
    private static function gerar_temporada($temporada_data)
    {
        ...
        foreach ($horarios_grupos as $horario => $dias) {
            ...
            $partes[] = "<span class='dias'>".$texto_dias . "</span> às <span class='horarios'>" . $horario . "h</span>";
        }
    ...
    {
        if ($temporada instanceof WP_Post) {
            $temporada_id = $temporada->ID;
        } elseif (is_numeric($temporada)) {
            $temporada_id = (int) $temporada; // Converte para int com segurança
        } else {
            return null;
        }
    ...
    public static function get_data_formatada($data)
    {
        ...
        $ano_atual = wp_date('Y'); // Pega o ano atual do sistema

        // Formatos de data traduzíveis (M retorna o mês abreviado com 3 letras)
        $formato_sem_ano = __('d \d\e M', 'cannal-espetaculos');
        $formato_com_ano = __('d \d\e M \d\e Y', 'cannal-espetaculos');

        $ts_inicio = strtotime($data);
        $ano = wp_date('Y', $ts_inicio);

        // Verifica se o ano da data é igual ao atual
        $formato = ($ano === $ano_atual) ? $formato_sem_ano : $formato_com_ano;

        return mb_strtolower(wp_date($formato, $ts_inicio), 'UTF-8');
    }

    public static function get_periodo_temporada($data_inicio, $data_fim)
    {
        $data_inicio_f = self::get_data_formatada($data_inicio);
        $data_fim_f = self::get_data_formatada($data_fim);

        if (! empty($data_inicio_f) && ! empty($data_fim_f)) {
            return sprintf(__('de %1$s até %2$s', 'cannal-espetaculos'), $data_inicio_f, $data_fim_f);
        }
        if (! empty($data_inicio_f)) {
            return sprintf(__('a partir de %s', 'cannal-espetaculos'), $data_inicio_f);
        }
        if (! empty($data_fim_f)) {
            return sprintf(__('até %s', 'cannal-espetaculos'), $data_fim_f);
        }
        return null;
    }
}
